...
const boxStageOne = () => {
    ...
    const [countdown, setCountdown] = useState(4); // 4 seconds for inhale in box breathing
    ...
    useEffect(() => {
        if (countdown === 0) {
            router.replace(`/(box)/boxStageHold?totalCycles=${totalCyclesNum}&currentCycle=${currentCycleNum}` as any);
        }
    }, [countdown]);
