...
const boxStageHold = () => {
    ...
    const [countdown, setCountdown] = useState(4); // 4 seconds for hold in box breathing
    ...
    useEffect(() => {
        if (countdown === 0) {
            router.replace(`/(box)/boxStageExhale?totalCycles=${totalCyclesNum}&currentCycle=${currentCycleNum}` as any);
        }
    }, [countdown]);
