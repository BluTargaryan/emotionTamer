...
const boxStageExhale = () => {
    const { totalCycles, currentCycle } = useLocalSearchParams();
    const [countdown, setCountdown] = useState(4); // 4 seconds for exhale in box breathing
    ...
    useEffect(() => {
        if (countdown === 0) {
            router.replace(`/(box)/boxStageHoldFinal?totalCycles=${totalCyclesNum}&currentCycle=${currentCycleNum}` as any);
        }
    }, [countdown]);
