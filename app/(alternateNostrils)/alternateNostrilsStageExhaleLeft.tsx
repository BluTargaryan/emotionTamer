...
const alternateNostrilsStageExhaleLeft = () => {
    const { totalCycles, currentCycle } = useLocalSearchParams();
    const [countdown, setCountdown] = useState(4); // 4 seconds for exhale
    ...
    useEffect(() => {
        if (countdown === 0) {
            if (currentCycleNum < totalCyclesNum) {
                // Move to next cycle
                router.replace(`/(alternateNostrils)/alternateNostrilsStageInhaleLeft?totalCycles=${totalCyclesNum}&currentCycle=${currentCycleNum + 1}` as any);
            } else {
                // All cycles completed
                router.replace(`/(alternateNostrils)/alternateNostrilsStageFinal?totalCycles=${totalCyclesNum}` as any);
            }
        }
    }, [countdown]);
...
export default alternateNostrilsStageExhaleLeft 