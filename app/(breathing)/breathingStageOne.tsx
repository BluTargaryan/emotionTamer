...
const breathingStageOne = () => {
    const { totalCycles, currentCycle } = useLocalSearchParams();
    const [countdown, setCountdown] = useState(4); // 4 seconds for inhale in 4-7-8 breathing
    ...
    useEffect(() => {
        if (countdown === 0) {
            router.replace(`/(breathing)/breathingStageHold?totalCycles=${totalCyclesNum}&currentCycle=${currentCycleNum}`);
        }
    }, [countdown]);
...
export default breathingStageOne        