...
const breathingStageHold = () => {
    const { totalCycles, currentCycle } = useLocalSearchParams();
    const [countdown, setCountdown] = useState(7); // 7 seconds for hold in 4-7-8 breathing
    ...
    useEffect(() => {
        if (countdown === 0) {
            router.replace(`/(breathing)/breathingStageTwo?totalCycles=${totalCyclesNum}&currentCycle=${currentCycleNum}`);
        }
    }, [countdown]);
...
export default breathingStageHold 